...
def get_surf_descriptors(X, sizes, img_grey):
    ...
    for i in range(len(X)):
        ...
        could_not_process = ((pt[0] - 10 * sigma_i < 0) or (pt[1] - 10 * sigma_i < 0) or
                             (pt[0] + 10 * sigma_i >= img_grey.shape[0]) or (pt[1] + 10 * sigma_i >= img_grey.shape[1]))
        ...
        Gx = cv2.Sobel(G, cv2.CV_64F, dx=1, dy=0, ksize=1)
        Gy = cv2.Sobel(G, cv2.CV_64F, dx=0, dy=1, ksize=1)
        Gx_r = Gx[pt[0] - 10 * sigma_i: pt[0] + 10 * sigma_i,
                  pt[1] - 10 * sigma_i: pt[1] + 10 * sigma_i].reshape(4, 4, -1)
        Gy_r = Gy[pt[0] - 10 * sigma_i: pt[0] + 10 * sigma_i,
                  pt[1] - 10 * sigma_i: pt[1] + 10 * sigma_i].reshape(4, 4, -1)
        holdD = np.array((Gx_r.sum(axis=2), Gy_r.sum(axis=2),
                          np.abs(Gx_r).sum(axis=2), np.abs(Gy_r).sum(axis=2)))
...
def find_correspondences(desc, X, sigmas, img_grey_2, thresh=0.95, max_disparity=30, min_disparity=0):
    ...
    for i in range(len(X)):
        ...
        for du in range(min_disparity, max_disparity):
            ...
            Gx_r = Gx[pt2[0] - ten_sigma_i: pt2[0] + ten_sigma_i,
                      pt2[1] - ten_sigma_i: pt2[1] + ten_sigma_i].reshape(4, 4, -1)
            Gy_r = Gy[pt2[0] - ten_sigma_i: pt2[0] + ten_sigma_i,
                      pt2[1] - ten_sigma_i: pt2[1] + ten_sigma_i].reshape(4, 4, -1)
            holdD = np.array((Gx_r.sum(axis=2), Gy_r.sum(axis=2),
                              np.abs(Gx_r).sum(axis=2), np.abs(Gy_r).sum(axis=2)))
            holdD /= np.linalg.norm(holdD, axis=0)[None]+1e-12
